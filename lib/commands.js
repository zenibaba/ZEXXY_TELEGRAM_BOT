...
async function handleBroadcasts() {
    ...
    let msg = `*📢 Active Broadcasts (${broadcasts.length})*\n\n`;

    broadcasts.forEach((b, index) => {
        const icon = b.active ? '🟢' : '🔴';
        msg += `${icon} *ID:* \`${b.id}\`\n`;
        msg += `🎯 Target: ${b.target}\n`;
        msg += `📅 Date: ${b.created_at.split('T')[0]}\n`;
        msg += `💬 "${b.message}"\n`;
        msg += `🗑️ /deletebroadcast ${b.id}\n\n`;
        msg += `-------------------\n\n`;
    });
...
async function handleUserInfo(args) {
    ...
    if (user.stats) {
        ...
        const limits = {
            'GUEST': '20k', 'USER': '20k',
            'VIP': '200k', 'PRO': '200k', 'ADMIN': '200k',
            'PARTNER': '100M', 'OWNER': 'Unlimited'
        };
        const limit = limits[user.rank] || '20k';

        msg += `📱 Generated: \`${user.stats.generated || 0}\` / ${limit}\n`;
